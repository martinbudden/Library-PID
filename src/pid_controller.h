...
public:
    inline void set_p(float p) { _pid.kp = p; }
    inline void set_i(float i) { _pid.ki = i; _ki_saved = _pid.ki; }
    inline void set_d(float d) { _pid.kd = d; }
    inline void set_s(float s) { _pid.ks = s; }
    inline void set_k(float k) { _pid.kk = k; }
    inline void set_pid(const pid_constants_t& pid) { _pid = pid; _ki_saved = _pid.ki; }
    inline float get_p() const { return _pid.kp; }
    inline float get_i() const { return _ki_saved; } // returns the set value of ki, whether integration is turned on or not
    inline float get_d() const { return _pid.kd; }
    inline float get_s() const { return _pid.ks; }
    inline float get_k() const { return _pid.kk; }
    inline const pid_constants_t get_pid() const { return pid_constants_t { _pid.kp, _ki_saved, _pid.kd, _pid.ks, _pid.kk }; }  // returns the set value of ki, whether integration is turned on or not

    inline void reset_integral() { _error_integral = 0.0F; }
    inline void switch_integration_off() { _ki_saved = _pid.ki; _pid.ki = 0.0F; _error_integral = 0.0F; }
    inline void switch_integration_on() { _pid.ki = _ki_saved; _error_integral = 0.0F; }
    ...
    inline void set_output_saturation_value(float output_saturation_value) { _output_saturation_value = output_saturation_value; }

    inline void set_setpoint(float setpoint) { _setpoint_previous = _setpoint; _setpoint = setpoint; }
    inline void set_setpoint(float setpoint, float delta_t) {
        _setpoint_previous = _setpoint;
        _setpoint = setpoint;
        _setpoint_derivative = (_setpoint - _setpoint_previous)/delta_t;
    }
    inline void set_setpoint_derivative(float setpoint_derivative) { _setpoint_derivative = setpoint_derivative; }

    inline float get_setpoint() const { return _setpoint; }
    inline float get_previous_setpoint() const { return _setpoint_previous; }
    inline float get_setpoint_delta() const { return _setpoint - _setpoint_previous; }

    inline float get_previous_measurement() const { return _measurement_previous; } //!< get previous measurement, useful for DTerm filtering
    ...
    void reset_all(); //!< reset all, for test code
private:
