...
5. Additional update function, `update_delta`, with a `measurementDelta` parameter. Providing this parameter
...
2. Option to calculate derivative on input. This can be achieved by calculating `inputDelta` rather than `measurementDelta`
   ...
   Note this value must be negated, ie `update_delta(measurement, -inputDelta, delta_t)` should be called.
3. Filtering of the D-term. Providing a D-term filter limits flexibility - the user no choice in the type of filter used.
   Instead the `update_delta` function can be used, with `measurementDelta` filtered by a filter provided by the user.
